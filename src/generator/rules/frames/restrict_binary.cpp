#include "restrict_binary.h"

#include "../../generator_data.h"


namespace dlplan::generator::rules {
void RestrictFrameBinary::generate_impl(const core::States& states, int target_complexity, GeneratorData& data, core::DenotationsCaches& caches) {
    core::SyntacticElementFactory& factory = data.m_factory;
    for (int i = 1; i < target_complexity - 1; ++i) {
        int j = target_complexity - i - 1;
        for (const auto& f : data.m_frames_binary_by_iteration[i]) {
...
std::string RestrictFrameBinary::get_name() const {
    return "f_restrict_binary";
}
